...
class yeEndDetail extends Component {
    ...
    onChange = (e) => {
     ...
     loadDepositSignList = () => {
        let input = {
            ...
            yeSignNo: window.localStorage.getItem("yeSignNo")
        }
        console.log('input 뜨나', input)
        DepositSignApi.fetchSignList(input)
            .then((res) => {
                this.setState({
                    signs: res.data,
                    yeJoinDate: res.data.yeJoinDate,
                    yeEndDate: res.data.yeEndDate,
                    yeAmount: res.data.yeAmount,             
            ...
            .catch(err => {
                console.log('loadDepositSignList() Error!!', err);
            })
    }
...
calculateInterest = () => {
    ...
    const yeAmount = parseFloat(this.state.yeAmount);
        const redeemDate = new Date(this.state.yeCancelDate);
        const interestRateMonth = interestRate / 12; // 연 이자율을 월 이자율로 변환합니다.

        // "시작일"과 "해지날짜" 사이의 월 차이를 계산합니다.
        const monthsDifference = (redeemDate.getFullYear() - yeJoinDate.getFullYear()) * 12 + (redeemDate.getMonth() - yeJoinDate.getMonth());
        const calculatedInterest = monthsDifference * yeAmount * interestRateMonth; // 계산된 이자를 구합니다.
...
payRequest = () =>{
    let input = {
        ...
        yeCancelDate : this.state.yeCancelDate,
        yeAmount : this.state.yeAmount
    }
    DepositSignApi.payRequest(input)
    .then(response => {
        ...
        if (response.data.success) {
            ...
            this.props.history.push('/depositSignList');
          } else {
            // 오류 메시지 처리
            alert(response.data.message);
        ...
        return (
            <div align='center'>
                <form>
                    ...
                    <Table style={{ width: '53vw' }}>
                        ...
                        <TableRow>
                            <TableCell style={{ textAlign: 'left', color: 'red', padding: '30px' }}>
                                ＊ 예금 해지 시 추가 본인확인 절차가 필요할 수 있으며, 해지 신청이 많아질 경우 추가 확인 절차에 시간이 소요될 수 있습니다. 양해 부탁드립니다.
                            </TableCell>
                        </TableRow>
                        <TableRow>
                            <TableCell style={{ textAlign: 'left', border: '1px solid rgb(230, 229, 227)', padding: '30px' }}>
                                ...
                                - 인터넷뱅킹을 통한 예금 해지 시, 해지 금액은 예금 신규 가입 시 사용한 출금 계좌(연결 계좌)로 입금됩니다.
                                - 연결된 계좌가 없는 경우, 해지 금액은 예금 개설 후 1개월 이상 지난 계좌로 입금됩니다.
                                - 입출식 계좌의 경우, 예금 개설 후 1개월 이상 경과하고 잔액이 100만원 이하인 예금(포함하여 MMDA 계좌)만 해지 가능합니다. 인터넷 해지가 불가한 경우, 가까운 영업점을 방문하여 해지할 수 있습니다.
                            </TableCell>
                        </TableRow>
                    </TableBody>
                            ...
                            <TableRow>
                                ...
                                <TableCell style={{ textAlign: 'center', border: '1px solid rgb(230, 229, 227)' }}>
                                    <TextField
                                        ...
                                        variant="standard"
                                        type="text"
                                        name="depositAccountNum"
                                        disabled
                                        value={this.state.signs.depositAccountNum} />
                                </TableCell>
                            </TableRow>
                            <TableRow>
                                <TableCell style={tableHead}>
                                ...
                                <TableCell style={{ textAlign: 'center', border: '1px solid rgb(230, 229, 227)' }}>
                                    {new Date(this.state.signs.yeJoinDate).toLocaleDateString(
                                        'ko-KR', {
                                        year: 'numeric',
                                        month: '2-digit',
                                        day: '2-digit'
                                ...
                                <TableCell style={{ textAlign: 'center', border: '1px solid rgb(230, 229, 227)' }}>
                                    {new Date(this.state.signs.yeEndDate).toLocaleDateString(
                                        'ko-KR', {
                                        year: 'numeric',
                                        month: '2-digit',
                                        day: '2-digit'
                            ...
                            <TableCell style={tableHead}>
                                ...
                                <TableCell style={{ textAlign: 'center', border: '1px solid rgb(230, 229, 227)' }}>
                                    <Input
                                        ...
                                        name="yeAmount"
                                        disabled
                                        value={this.state.signs.yeAmount}
                                    />
                                </TableCell>
                    ...
                    <span style={{ color: "red", fontSize: '13px' }}>※ 예금 중도상환 정보를 확인 하였으며, 동의합니다. ※</span>
                    <Checkbox
