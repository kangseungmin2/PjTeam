	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		http
			...
			.authorizeHttpRequests((requests) -> requests
					.antMatchers(HttpMethod.POST, "/login", "/register","/fund").permitAll()
					.antMatchers(HttpMethod.GET, "/fund").permitAll()
					.antMatchers(HttpMethod.DELETE, "/fund").permitAll()
					.antMatchers(HttpMethod.PUT, "/fund").permitAll()// 인증이 필요하지 않은 유일한 엔드포인트이며, 리액트의 반드시 url 일치시키기
